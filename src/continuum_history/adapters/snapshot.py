...
def load_snapshot(path: Path) -> Snapshot:
    ...
    # Compare each API with itself: Windows path/handle stat metadata can differ.
    # The path pair detects replacement; the handle pair detects writes during the read.
    pairs = ((before, after), (path_before, current))
    if any(getattr(old, f) != getattr(new, f) for old, new in pairs for f in fields):
